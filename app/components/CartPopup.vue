<template>
  <div :class="[
    ...
    <div class="absolute top-0 left-0 w-full h-full bg-black/70" @click="closePopupCart"></div>
    <div :class="[
      ...
      <div class="flex justify-between items-center h-20 px-5 bg-white border-b border-[#E6E9F0]">
        <div class="w-[170px]">
        ...
        <div class="flex items-center gap-3">
          <span class="text-xs text-[#7B7B7B]">{{ selectedCount }}/{{ cartData?.cart?.length || 0 }}</span>
          <button class="relative cursor-pointer h-10 w-10 flex-shrink-0 bg-transparent border-none group"
      ...
      <div class="h-[calc(100vh-80px-140px)] px-5 py-5 overflow-y-auto bg-white flex-1">
        <!-- Loading State -->
        <template v-if="isLoading">
          <div class="mb-4">
            <div class="border border-[#E6E9F0] rounded-lg p-3 mb-3" v-for="i in 3" :key="'cartSkeleton' + i">
              <div class="flex flex-col">
                <div class="flex mb-3">
                  <div class="w-[78px] h-[78px] mr-3 flex-shrink-0">
                    <div class="w-full h-full aspect-square rounded-lg bg-gray-200 animate-pulse"></div>
                  </div>
                  <div class="flex-1">
                    <div class="w-[150px] h-4 bg-gray-200 rounded mb-2 animate-pulse"></div>
                    <div class="w-[120px] h-3 bg-gray-200 rounded mb-2 animate-pulse"></div>
                    <div class="w-[100px] h-3 bg-gray-200 rounded animate-pulse"></div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <template v-else>
          ...
          ">
            <div class="mb-4">
              <div v-if="cartData.cart && cartData.cart.length > 0"
                ...
                <span class="text-sm font-medium text-[#1A1919]">Pilih semua</span>
              </div>

              <!-- Semua item (normal + invalid) -->
              <div v-for="item in allCartItems" :key="item.variant_id"
                class="border rounded-lg p-3 mb-3 bg-white transition" :class="[
                  item.isInvalid
                ...
                <div class="flex flex-col">
                  <div class="flex mb-3">
                    ...
                    <label class="flex items-center cursor-pointer mr-2 flex-shrink-0">
                      <input type="checkbox" :checked="isSelectedForCheckout(item.variant_id)"
                        @change="toggleSelectedForCheckout(item.variant_id)" :disabled="item.isInvalid"
                    ...
                    <component :is="item.isInvalid ? 'div' : 'NuxtLink'" :to="!item.isInvalid
                      ...
                      :class="{ 'cursor-not-allowed': item.isInvalid }" @click="!item.isInvalid && closePopupCart()">
                      <NuxtImg v-if="item.image" :src="item.image" :alt="item.product_name"
                    ...
                    <!-- Info -->
                    <div class="flex-1">
                      <component :is="item.isInvalid ? 'p' : 'NuxtLink'" :to="!item.isInvalid
                          ? `/products/${item.product_slug || item.product_id}`
                          : undefined
                        " class="font-semibold mb-1 text-sm" :class="[
                          item.isInvalid
                  ...
                  <div class="flex flex-col mt-3">
                      ...
                      <textarea
                        ...
                        :class="{
                          'bg-gray-100 cursor-not-allowed opacity-70':
                            item.isInvalid,
                        }" rows="2" :value="item.note || ''" @blur="onChangeNote($event, item.variant_id)"
                        :disabled="item.isInvalid" placeholder="Tambahkan catatan..."></textarea>
                    </div>
                    ...
                    <div class="flex items-center justify-between">
                      ...
                      <div
                        ...
                        <button @click="minQty(item.variant_id)" :disabled="item.qty <= 1"
                          ...
                          <svg width="20" height="20" class="sm:w-6 sm:h-6" viewBox="0 0 24 24" fill="none"
                            ...
                            <path d="M19 12.998H5V10.998H19V12.998Z" fill="#C1C1C1" />
                          </svg>
                        </button>
                        <span class="text-sm sm:text-base text-[#1A1919]">{{
                          item.qty
                        }}</span>
                        <button @click="plusQty(item.variant_id)" :disabled="item.qty >= item.available_stock"
                          class="hover:cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
                          <svg width="20" height="20" class="sm:w-6 sm:h-6" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path d="M19 12.998H13V18.998H11V12.998H5V10.998H11V4.99805H13V10.998H19V12.998Z" :fill="item.qty >= item.available_stock
                                ? '#C1C1C1'
                                : '#E9322B'
                              " />
                          </svg>
                      ...
                      <!-- Hapus tetap aktif -->
                      <button class="p-0 hover:text-red-600 transition hover:cursor-pointer"
          ...
          <!-- Empty Cart -->
          <template v-else>
            <div class="flex flex-col items-center justify-center text-center h-[300px]">
      ...
      <div class="h-[140px] w-full px-5 py-5 border-t border-[#E6E9F0] bg-white"
        ...
        <div class="flex gap-2">
          <NuxtLink to="/cart" @click="closePopupCart"
